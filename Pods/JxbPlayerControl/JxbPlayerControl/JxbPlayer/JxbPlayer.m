...
- (void)setUI {
    ...
    _slider = [[UISlider alloc] initWithFrame:CGRectMake(20, playerIconWidth + 30, self.frame.size.width - 40, 30)];
    ...
    _vDownProgress = [[UIView alloc] initWithFrame:CGRectMake(2, 14, 0, 2)];
...
- (void)play {
    ...
    self.avItem = [AVPlayerItem playerItemWithAsset:movieAsset];
    [self addObserver];
...
- (void)addObserver
{
    [self.avItem addObserver:self forKeyPath:@"status" options:NSKeyValueObservingOptionNew context:nil];// 监听status属性
    [self.avItem addObserver:self forKeyPath:@"loadedTimeRanges" options:NSKeyValueObservingOptionNew context:nil];// 监听loadedTimeRanges属性
}
...
- (void)playing {
    ...
    if (avDuration > 0)
    {
        ...
        _vDownProgress.frame = CGRectMake(2, 14, download / avDuration * _slider.frame.size.width, 2);
    }
...
//    }
    if(_delegate && [_delegate respondsToSelector:@selector(XBPlayer_pre)])
        [_delegate XBPlayer_pre];
...
//        [_avPlayer seekToTime:CMTimeMakeWithSeconds((t+20), playerScale)];
//    }
    if(_delegate && [_delegate respondsToSelector:@selector(XBPlayer_next)])
        [_delegate XBPlayer_next];
}
...
- (void)seekToTime:(CGFloat)time
{
    ...
    [_avPlayer seekToTime:CMTimeMakeWithSeconds(time, playerScale)];
