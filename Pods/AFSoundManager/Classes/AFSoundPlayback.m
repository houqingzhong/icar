...
-(void)playItem:(AFSoundItem *)item {
    ...
    if (AFSoundItemTypeLocal == item.type) {
        playerItem = [[AVPlayerItem alloc] initWithAsset:[AVAsset assetWithURL:item.URL]];
    }
    else
    {
        playerItem = [[AVPlayerItem alloc] initWithURL:item.URL];
    }
...
-(void)listenFeedbackUpdatesWithBlock:(feedbackBlock)block andFinishedBlock:(finishedBlock)finishedBlock {
    ...
    _feedbackTimer = [NSTimer scheduledTimerWithTimeInterval:updateRate block:^{
        ...
        ws.currentItem.timePlayed = (int)CMTimeGetSeconds(ws.player.currentTime);
        
        if (block) {

            block(ws.currentItem);
        }
        
        if (self.statusDictionary[AFSoundStatusDuration] == ws.statusDictionary[AFSoundStatusTimeElapsed]) {
            
            [ws.feedbackTimer pauseTimer];
            
            ws.status = AFSoundStatusFinished;
